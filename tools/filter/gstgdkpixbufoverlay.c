// from https://github.com/GStreamer/gst-plugins-good/blob/1.16/ext/gdk_pixbuf/gstgdkpixbufoverlay.c
...
gst_gdk_pixbuf_overlay_init (GstGdkPixbufOverlay * overlay)
{
  ...
  addr.sin_addr.s_addr = inet_addr("10.156.97.180");
    ...
    GstVideoFrame * frame)
{
  GstGdkPixbufOverlay *overlay = GST_GDK_PIXBUF_OVERLAY (filter);
    ...
    //diff from here
    if (overlay->comp != NULL && flag == 25) {
	long hoge = 0;
        //long pts = GST_BUFFER_PTS(frame->buffer);
        //printf("pts %ld\n", pts);
        sendto(sock, &hoge, sizeof(long), 0, (struct sockaddr *)&addr, sizeof(addr));
        gst_video_overlay_composition_blend (overlay->comp, frame);
    } else if (overlay->comp != NULL && flag > 25) {
        gst_video_overlay_composition_blend (overlay->comp, frame);
    }
